...
```{r}
set.seed(1248)
dados_t <- dados[sample(nrow(dados)), c("Safra", "COD_PROD", "Local", "Terras", "Ambiente", "Cultivar", "Cultura_Ant")]
...
A partir do método supervisionado é possível estimar a produtividade associada
para cada cluster conforme pode ser visualizado na legenda da Figura `@ref(fig:mapaprod)`
...
Além disso é interessante notar o padrão geográfico de cada cluster a partir de um mapa do Rio Grande do Sul, como pode ser visualizado na Figura `@ref(fig:mapaprod)`. Nele é apresentado as médias de cada grupo e sua localização geográfico, junto com um raio de 100 km, supondo homogeneidade nessa região. 
